"""Embedded media: aspect-correct ``data-aspect`` placement.
...
def test_contain_letterboxes_a_wide_source_in_a_square_box(tmp_path):
    ...
    doc = parse_file(_comp(tmp_path, 'data-aspect="contain"'))
...
def test_cover_fills_the_box_and_is_clipped_to_it(tmp_path):
    ...
    doc = parse_file(_comp(tmp_path, 'data-aspect="cover"', box=(50, 50, 100, 100)))
    node = _image_node(bake.bake_tree(doc, 0.0))
    assert (node.get("width"), node.get("height")) == ("200", "100")
...
def test_stretch_and_absent_fit_leave_the_geometry_alone(tmp_path):
    ...
    for attrs in ("", 'data-aspect="stretch"'):
...
def test_unknown_aspect_mode_is_reported_by_lint(tmp_path):
    comp = _comp(tmp_path, 'data-aspect="fill"')
    codes = [f.code for f in lint_string(open(comp).read())]
    assert "image.bad_aspect" in codes


def test_known_aspect_modes_are_not_reported(tmp_path):
    for mode in media.ASPECT_MODES:
        comp = _comp(tmp_path, f'data-aspect="{mode}"')
        codes = [f.code for f in lint_string(open(comp).read())]
        assert "image.bad_aspect" not in codes
